...
lsblk
```
Create your partitions (EFI, root, swap), then double-check they look right.
...
swapon /dev/sda2
```
Root mounted to `/mnt`, EFI mounted into `/mnt/boot/efi`, swap turned on.
...
arch-chroot /mnt
```
Installs the base system and essentials, writes the fstab, then drops you into the new system. Everything below runs inside chroot.
...
nano /etc/hostname
```
Set your timezone, sync the hardware clock, uncomment and generate your locale, set `LANG`, set the TTY keyboard layout, and name your machine.
